...
abstract class Model {
	 ...
	 * Model join.
	 */
	private function _join_model($model_string, $our_join_attribute = "id", $other_join_attribute = "id", $type = "INNER", $operator = "=") {
		// Pass the method to the query instance
		$this->query->join(static::table($model_string) . ' as ' . $model_string, static::$table . '.' . $our_join_attribute, $operator, $model_string . '.' . $other_join_attribute, $type);
		$this->query->select(static::$table . '.*');
	...
	public function save()
	{
        ...
        if($success) {
            \Events::launch(EVENT_ENTITY_SAVED, ['id' => $this->id, 'class' => get_class($this)]);
        }
	...
	protected function timestamp()
	{
		...
		if ( ! $this->exists) $this->created_at = $this->attributes['updated_at'];
	}
	...
	public function __get($key)
	{
		...
		elseif (method_exists($this, $key))
		{
            ...
            $relating_key = $this->relating_key;

            if($relating == 'belongs_to') {
                return $this->ignore[$key] = $this->attributes[$relating_key] == null ? null : $query->first();
            }

            return $this->ignore[$key] = $relating == 'has_one' ? $query->first() : $query->get();
		}
	}
	...
	public function __call($method, $parameters)
	{
		...
		if (in_array($method, array('with', 'get', 'first', 'paginate', 'join_model')))
		{
    ...
    public function __sleep() {
        return array(
            'exists', 'attributes', 'dirty', 'ignore', 'includes', 'relating', 'relating_key', 'relating_table'
            );
    }
