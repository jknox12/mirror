...
class Lang {
	 ...
	 * @return bool
	 */
	protected function load($file, $language = null)
	{
        if (is_null($language)) $language = $this->language;

		if (isset(static::$lines[$language][$file])) return true;

		$lines = array();

		if (file_exists($path = LANG_PATH.$language.'/'.$file.EXT)) {
			$lines = array_merge($lines, require $path);
		} else {
		    $path = MWP_CORE_PATH . "lang/$language/$file.php";
		    if (file_exists($path)) {
			    $lines = array_merge($lines, require $path);
		    }
		}
		...
		if (count($lines) > 0)
		{
			static::$lines[$language][$file] = $lines;
		}
		
		return isset(static::$lines[$language][$file]);
	}
