...
class URI {
	...
	public static function current()
	{
		...
		if (substr($requestPath, 0, 7) == '/http:/'){
			$requestPath = strstr(substr($requestPath, 7), '/');
		}

		$uri = @parse_url($requestPath, PHP_URL_PATH);
	...
	public static function current_stripped() {
        static::current();
        return implode('/', array_slice(static::$segments, 2));
	}

    public static function api_version() {
        return static::segment(1);
    }

	public static function store_id() {
		return intval(static::segment(2));
	}
