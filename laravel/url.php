...
class URL {
	...
	{
        //Fix for filter var
		$url_to_validate = str_replace('-', '', $url);
        
		if (filter_var($url_to_validate, FILTER_VALIDATE_URL) !== false) return $url;
	 ...
	 * @return string
	 */
	public static function to_route($name, $parameters = array(), $https = null)
	{
		if (is_null($https)) $https = Config::$items['application']['ssl'];
